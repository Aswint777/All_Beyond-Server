...
export class NotificationUsCase {
  ...
  async readAllNotificationsUseCase(userId: string): Promise<bool | null> {
    const { readAllNotificationsRepo } = this.dependencies.repositories;
    try {
      return await readAllNotificationsRepo(userId);
    } catch (error: constant) {
      console.error("Error in getNotificationsUseCase:", error);
      throw new Error(
        "An unexpected error occurred: " + (error.message || "Unknown error")
  ...
  async assessmentNotificationUpdate(
    assessmentId: string
